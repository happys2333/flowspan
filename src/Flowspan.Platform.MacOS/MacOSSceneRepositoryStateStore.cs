...
public sealed class MacOSSceneRepositoryStateKeyStore :
    ISceneRepositoryStateKeyStore
{
    ...
    public const string DefaultService =
        "app.flowspan.scene-repository-state-key";
    private readonly string account;
    private readonly IMacOSKeychain keychain;
    private readonly string service;

    public MacOSSceneRepositoryStateKeyStore()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        byte[] winner = keychain.LoadGenericPassword(service, account)
            ?? throw new IOException(
                "The macOS Scene repository state key disappeared after a concurrent create.");
    ...
    private static byte[] RequireValidKey(byte[] key)
    {
        ...
        throw new InvalidDataException(
            "The macOS Keychain Scene repository state key has an invalid length.");
    }
...
public sealed class MacOSSceneRepositoryStatePayloadStore :
    ISceneRepositoryStatePayloadStore
{
    private readonly AuthenticatedSceneRepositoryStateFile inner;

    public MacOSSceneRepositoryStatePayloadStore()
        : this(GetDefaultStatePath(), new MacOSSceneRepositoryStateKeyStore())
    {
    }
    ...
    public MacOSSceneRepositoryStatePayloadStore(
        string statePath,
        IMacOSKeychain keychain)
        : this(statePath, new MacOSSceneRepositoryStateKeyStore(keychain))
    {
    }
