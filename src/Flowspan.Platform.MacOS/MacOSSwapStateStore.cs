...
public sealed class MacOSSwapStateKeyStore : ISwapStateKeyStore
{
    public const string DefaultAccount = "primary-swap-state-key";
    public const string DefaultService = "app.flowspan.swap-state-key";
    private readonly string account;
    private readonly IMacOSKeychain keychain;
    private readonly string service;

    public MacOSSwapStateKeyStore()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        byte[] winner = keychain.LoadGenericPassword(service, account)
            ?? throw new IOException(
                "The macOS Swap state key disappeared after a concurrent create.");
    ...
    private static byte[] RequireValidKey(byte[] key)
    {
        ...
        throw new InvalidDataException(
            "The macOS Keychain Swap state key has an invalid length.");
    }
...
public sealed class MacOSSwapStatePayloadStore : ISwapStatePayloadStore
{
    private readonly AuthenticatedSwapStateFile inner;

    public MacOSSwapStatePayloadStore()
        : this(GetDefaultStatePath(), new MacOSSwapStateKeyStore())
    {
    }
    ...
    public MacOSSwapStatePayloadStore(
        string statePath,
        IMacOSKeychain keychain)
        : this(statePath, new MacOSSwapStateKeyStore(keychain))
    {
    }
