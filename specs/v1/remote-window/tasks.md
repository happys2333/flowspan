...
- [x] 2. Implement the portable live-session control plane
  ...
  - Final workflow tests prove failed explicit and Capability-driven peer
    ...
    the shared 16-slot session budget. Exact portable/headless evidence is in
    `docs/evidence/2026-08-10-desktop-remote-window-workflow.md`; native and
    physical evidence remains in task 9.
...
- [x] 3. Integrate protection and local emergency stop
  ...
  - Monotonic protection revisions, bounded re-entrant convergence, late-start
    ...
    coverage in the final portable/headless workflow. Exact automated evidence
    is recorded; native and physical evidence remains in tasks 6-9.
...
- [x] 4. Freeze authenticated control and bounded media protocol
  ...
  - Local macOS evidence on 2026-08-08: locked restore, format verification,
    ...
    vulnerability query all passed. Exact hosted evidence is recorded in task
    9; native and physical evidence remains there.
  - _Requirements: RW1-RW8_

- [x] 5. Implement Desktop Remote Window and Mirror workflow
  ...
  - [x] Record exact local and hosted evidence without treating fake or hosted
    platform contracts as native permission/capture/input proof.
    - Final commit `e34e73339dbb1c1ccf9de0b047653ddc5d7fbb59`
      passed the 1542-test local gate and exact-commit Windows, macOS, and Ubuntu
      CI, Secret Scan, CodeQL, and three-RID reproducible unsigned-package
      matrix. Downloaded TRX, SARIF, and package artifacts were independently
      parsed, hashed, and verified; exact IDs, digests, and limitations are in
      `docs/evidence/2026-08-10-desktop-remote-window-workflow.md`.
...
- [ ] 9. Close Task 6 evidence
  ...
  - Portable commit `d19cfea8a06dfec13d298ba2630916dc5e3bbf33`
    ...
    `docs/evidence/2026-08-08-portable-remote-window-control-plane.md`.
  - Desktop workflow commit `e34e73339dbb1c1ccf9de0b047653ddc5d7fbb59`
    also passed 1542 tests on each hosted OS, Secret Scan, CodeQL, and the
    matching reproducible unsigned package matrix. Parsed evidence is in
    `docs/evidence/2026-08-10-desktop-remote-window-workflow.md`.
