...
- [-] 3.3 Implement atomic swap prepare/reserve/decision/commit/recovery state
  ...
  - _Requirements: R5_
- [x] 3.4 Add generated transition/property cases and deterministic drop,
  duplicate, delay, disconnect, and journal-failure tests.
  - Implementation commit `c590e1a` adds 4,096 seeded Operation-journal events,
    64 composed Move delivery traces, 4,096 seeded Mirror/lease transitions,
    and explicit Handoff pre-write plus Handoff/Move ambiguous post-write
    journal-failure
    ...
    Exact implementation commit `c590e1a` passes CI run `29488721383` on
    Windows, macOS, and Ubuntu, Secret Scan, CodeQL run `29488721396`, and
    independently summed downloaded TRX verification at 713/713 passed per OS.
    Evidence commit `4799dda` passes the same gates in CI run `29489188380` and
    CodeQL run `29489188306`; its three downloaded 11-TRX artifacts again sum to
    713/713 passed per OS. This status becomes final only after the task-status
    HEAD passes the same workflows. Release-wide properties for unimplemented
    Groups, Scenes, Remote Window, native Adapters, and physical faults remain
    open.
...
- [-] 4.3 Implement encrypted framed sessions and negative tests for tamper,
  ...
  - Directional AEAD frames and a four-message authenticated ephemeral TCP
    ...
    testing remain open.
  - [x] 4.3a Add protocol-1.2 bidirectional encrypted Finished before control
    ...
    fixture, tamper/binding/timeout negatives, and real loopback evidence.
    - Current candidate freezes a 62-byte Finished plaintext and golden hash,
      consumes each direction's epoch-1 sequence zero before control upgrade,
      maps tamper/binding failure to `InvalidPeerFinished`, bounds omission by
      the handshake deadline, and makes production discovery/reconnect prefer
      protocol 1.2 while keeping tested 1.1 compatibility. All 734 local tests
      pass; commands, proof mapping, and explicit rekey/review/physical limits
      are recorded in
      ...
      summed 734/734 downloaded TRX evidence. Evidence commit `fd73fa7` passes
      CI run `29494260925`, CodeQL run `29494260905`, Secret Scan, and a second
