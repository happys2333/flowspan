...
Commit `80191d6` provides six narrow managed same-host production-path tracer
scenarios: successful DriverEligible media/input/Emergency Stop; reversed-grant
denial; terminal authenticated-control disconnect; same-session Mirror
capability revocation; managed active permission loss; and a signed, verified
candidate whose `FSM1` TCP endpoint refuses connection before any attachment
request bytes are sent. Local exact-tree Debug and Release verification each
pass 2209/2209 solution tests, including Desktop 535/535 and Transport 687/687.
