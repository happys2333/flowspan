...
P0/P1/P2 finding but is not an external audit. The commit has not been pushed,
so exact-SHA hosted CI and CodeQL remain pending and no hosted result is claimed.

This closes only one post-attachment timeout example. Actual caller
cancellation, cleanup-fault coverage, and the complete per-boundary matrix
remain open.
