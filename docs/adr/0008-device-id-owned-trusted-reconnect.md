...
- requires the local `activity.offer` grant used by the current idle handler;
...
capability checks. Consequently the idle channel exists only when both endpoints
still authorize it. A conflicting discovery fingerprint is blocked and warned
about, never used to change the election or replace Trust.
...
- Both peers must have local networking explicitly enabled, and the required
  grant must remain valid on both sides.
...
- The current channel only monitors authenticated transport closure. It performs
  no Activity operation and remains labelled `AUTHENTICATED — IDLE / NOT
  SHARING`.

## Evidence and limits

Deterministic coordinator tests cover both election sides, grant reconciliation,
identity-warning latching, retry progress, revoke, and drain. A same-process
loopback test runs the production authenticated connector and listener and
observes the idle status on both peers. Hosted matrix results can prove these
contracts execute on Windows, macOS, and Linux runners; they do not prove
